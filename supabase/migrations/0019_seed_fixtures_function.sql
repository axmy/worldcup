-- ============================================================
-- Runtime re-seed of the built-in WC2026 schedule (no API needed)
-- ============================================================
-- After an admin "Reset data" (which deletes all fixtures), or when the live
-- data API isn't available, this lets an admin re-insert the same 72 group-stage
-- fixtures that migration 0006 seeds at install time. Admin-guarded; only seeds
-- when the matches table is empty (so it never duplicates).
...
begin
  ...
  if exists (select 1 from public.matches) then
    return 0;  -- already populated — refuse to duplicate
  end if;
...
grant execute on function public.seed_wc2026_fixtures() to authenticated;
